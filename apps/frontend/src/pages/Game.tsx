...
export function GamePage() {
  ...
  const { setTotalRounds, setGameState, setCurrentRound, resetGame } =
    useGameStore();
  ...
  useEffect(() => {
    if (sessionState) {
      ...
      setGameState(
        sessionState.session.status === "finished" ? "game-end" : "playing",
      );
    } else if (session) {
  ...
  return (
    <div className="min-h-screen bg-background crt-scanlines">
      <ConnectionStatus />
      <div className="container mx-auto p-4 pt-16 pb-8">
