...
export const GameBoard: React.FC = () => {
  ...
  const [playerHand, setPlayerHand] = useState<Array<CardType | null>>([null, null, null])
  ...
  return (
    <div className="flex flex-col items-center justify-center py-8">
      <Card className="w-full max-w-2xl p-8 shadow-lg">
        <h1 className="text-3xl font-bold text-center mb-8 font-heading neon-text">Three Card Poker</h1>
        ...
        <div className="space-y-6">
          <div className="flex justify-center gap-4">
            {playerHand.map((card, index) => (
              <button
                ...
                aria-label={card ? `Card ${card.rank}${card.suit}` : `Draw card ${index + 1}`}
              >
                {card ? (
                  <>
                  ...
                  <span className="text-sm font-semibold text-slate-200 font-body">Draw</span>
                )}
              </button>
            ))}
          </div>
          ...
          <p className="text-center text-muted-foreground font-body">
            {score === null ? 'Click each card to draw your hand' : 'Hand complete'}
          </p>
          ...
          {score !== null && (
            <div className="text-center space-y-3">
              ...
              {hasPublished ? (
                <p className="text-sm text-green-600 font-body">Score published!</p>
              ) : (
                <Button
                  onClick={publishScore}
                  className="font-body"
                  disabled={!player || !session || publishScoreMutation.isPending}
                >
                  Publish Score
                </Button>
              )}
